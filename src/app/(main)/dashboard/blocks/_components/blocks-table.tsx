...
export function BlocksTable() {
  ...
  const { data, isLoading } = useContent("blocks", { pageSize: 200 });
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setEditBlock(row.original); }}>
            <Pencil className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setDeleteBlock(row.original); }}>
  ...
  return (
    <>
      <DataTable columns={columns} data={blocks} isLoading={isLoading} searchKey="title" searchPlaceholder="Search blocks..." />
      {editBlock && <BlockFormDialog open={!!editBlock} onOpenChange={(o) => !o && setEditBlock(null)} block={editBlock} />}
      {deleteBlock && (
        <BlockDeleteDialog
          open={!!deleteBlock}
