...
export function BannersTable() {
  ...
  const { data, isLoading } = useContent("banners", { pageSize: 200 });
    ...
    { accessorKey: "sort_order", header: "Sort" },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setEditBanner(row.original); }}>
            <Pencil className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setDeleteBanner(row.original); }}>
  ...
  return (
    <>
      <DataTable columns={columns} data={banners} isLoading={isLoading} searchKey="title" searchPlaceholder="Search banners..." />
      {editBanner && <BannerFormDialog open={!!editBanner} onOpenChange={(o) => !o && setEditBanner(null)} banner={editBanner} />}
      {deleteBanner && (
        <BannerDeleteDialog
          open={!!deleteBanner}
